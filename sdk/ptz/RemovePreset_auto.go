// Code generated : DO NOT EDIT.

package ptz

import (
	"context"

	"github.com/av1ppp/onvif"
	"github.com/av1ppp/onvif/sdk"
	"github.com/av1ppp/onvif/ptz"
	"github.com/av1ppp/onvif/errors"
)

// RemovePreset forwards the call to onvif.Do then parses the payload of the reply as a RemovePresetResponse.
func RemovePreset(ctx context.Context, dev *onvif.Device, request *onvif.Req[ptz.RemovePreset]) (ptz.RemovePresetResponse, error) {
	type Envelope struct {
		Header struct{}
		Body   struct {
			RemovePresetResponse ptz.RemovePresetResponse
		}
	}
	var reply Envelope

	httpReply, err := onvif.Do(dev, request)
	if err != nil {
		return reply.Body.RemovePresetResponse, errors.Common.Wrap(err, "failed to call method").WithProperty(errors.PropMethod, "RemovePreset")
	}

	logger := dev.GetLogger()
	if logger != nil {
		err = sdk.ReadAndParseWithLogging(ctx, logger, httpReply, &reply, "RemovePreset")
	} else {
		err = sdk.ReadAndParse(ctx, httpReply, &reply)
	}
	return reply.Body.RemovePresetResponse, err
}
